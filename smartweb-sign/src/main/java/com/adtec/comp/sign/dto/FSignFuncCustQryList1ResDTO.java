package com.adtec.comp.sign.dto;

public class FSignFuncCustQryList1ResDTO {
	private String ACCT;
	private String CUR_MAIN_ACCT;
	private String ACCT_NAME;
	private String ACCT_TP;
	private String CERT_TP;
	private String CERT_NO;
	private String PHONE_NO;
	private String COMM_ADDR;
	private String EMAIL_ADDR;
	public String getACCT() {
		return ACCT;
	}
	public void setACCT(String aCCT) {
		ACCT = aCCT;
	}
	public String getCUR_MAIN_ACCT() {
		return CUR_MAIN_ACCT;
	}
	public void setCUR_MAIN_ACCT(String cUR_MAIN_ACCT) {
		CUR_MAIN_ACCT = cUR_MAIN_ACCT;
	}
	public String getACCT_NAME() {
		return ACCT_NAME;
	}
	public void setACCT_NAME(String aCCT_NAME) {
		ACCT_NAME = aCCT_NAME;
	}
	public String getACCT_TP() {
		return ACCT_TP;
	}
	public void setACCT_TP(String aCCT_TP) {
		ACCT_TP = aCCT_TP;
	}
	public String getCERT_TP() {
		return CERT_TP;
	}
	public void setCERT_TP(String cERT_TP) {
		CERT_TP = cERT_TP;
	}
	public String getCERT_NO() {
		return CERT_NO;
	}
	public void setCERT_NO(String cERT_NO) {
		CERT_NO = cERT_NO;
	}
	public String getPHONE_NO() {
		return PHONE_NO;
	}
	public void setPHONE_NO(String pHONE_NO) {
		PHONE_NO = pHONE_NO;
	}
	public String getCOMM_ADDR() {
		return COMM_ADDR;
	}
	public void setCOMM_ADDR(String cOMM_ADDR) {
		COMM_ADDR = cOMM_ADDR;
	}
	public String getEMAIL_ADDR() {
		return EMAIL_ADDR;
	}
	public void setEMAIL_ADDR(String eMAIL_ADDR) {
		EMAIL_ADDR = eMAIL_ADDR;
	}
	
}
