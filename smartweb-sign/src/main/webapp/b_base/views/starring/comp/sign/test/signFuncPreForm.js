console.log('signFuncPreForm.js');
...
function sendData() {
	if (proof()&&flg==0) {
		...
		Ewin.confirm({
			...
			var $url = ctx;
			$url += "/comp/sign/test/signFuncCust/preSign";
			var $data = getData();
			$.ajax({
				url : $url,
				type : "POST",
		...
		$("#PROT_END_DATE").blur();
		goTop();
		$('#custPreSignForm').find('span.warnBlock:first').prev().parent().find('input:first').focus();
	}
}
function getData() {
	var rowArr = $('#table').bootstrapTable('getData');
	...
	if (SAVE_OR_REV == "add") {
		var addData = {
			/*SIGN_TP:getS("SIGN_TP"),
			SIGN_PROT_TP_NAME : getST("SIGN_PROT_TP_NAME").split("|")[0],*/
...
function checkEndTime() {
	...
	var my_Date = year+slash+month+slash+ri;
	var myDate = Date.parse(my_Date);
	if(startTime_one=="" || myDate<=startTime_one){ 
			var startTimeId = $("[check-startTime-own]").attr("id");
			$("#"+startTimeId).addClass('trueInput');
			$("#"+startTimeId).parent().siblings('span.warnBlock').remove();
			flg=0;
