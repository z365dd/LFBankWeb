 ...
 */
@Component
public class TPipClrMertFeeCaltDao implements IBaseDao<TPipClrMertFeeCaltDO>{
	/**
	 * 日志对象
	 */
	protected final static Logger log = LoggerFactory.getLogger(TPipClrMertFeeCaltDao.class);
	/*表名称*/
    public static final String TABLE_NAME = "T_PIP_CLR_MERT_FEE_CALT";
	 ...
	 */
	@Override
    public TPipClrMertFeeCaltDO get(TPipClrMertFeeCaltDO obj) {
        StringBuilder sql = new StringBuilder();
		sql.append("select * from ").append(TABLE_NAME).append(" where rule_id=?");
        TPipClrMertFeeCaltDO rs = null;
        IDBSession session = DBSessionFactory.getSession();
        try {
            rs = session.getObject(sql.toString(), TPipClrMertFeeCaltDO.class, obj.getRuleId());
        } catch (Exception e) {
	 ...
	 */
	@Override
	public int insert(TPipClrMertFeeCaltDO obj){
		int rs = 0;
		IDBSession session = DBSessionFactory.getSession();
		try {
	 ...
	 */
	@Override
	public int update(TPipClrMertFeeCaltDO obj){
		int rs = 0;
		IDBSession session = DBSessionFactory.getSession();
		try {
			List<String> ignoreFields = obj.getIgnoreFields();
	...
	public int delete(String id){
		TPipClrMertFeeCaltDO obj = new TPipClrMertFeeCaltDO();
		obj.setRuleId(id);
		return delete(obj);
	}
	 ...
	 */
	@Override
	public int delete(TPipClrMertFeeCaltDO obj){
		int rs = 0;
		IDBSession session = DBSessionFactory.getSession();
		try {
	 ...
	 */
	@Override
	public List<TPipClrMertFeeCaltDO> list(TPipClrMertFeeCaltDO obj) {
	 ...
	 */
	@Override
	public List<TPipClrMertFeeCaltDO> list(TPipClrMertFeeCaltDO obj, int start, int limit) {
		log.debug("TPipClrMertFeeCaltDO=" + obj.toString());
		log.debug("start=" + start);
		log.debug("limit=" + limit);
		List<TPipClrMertFeeCaltDO> list = null;
		try {
        	...
        	IDBSession session = DBSessionFactory.getSession();
			if (limit == 0) {
				list = session.getObjectListByList(sql.toString(), TPipClrMertFeeCaltDO.class, parameters);
			} else {
				list = session.getObjectListByListForPage(sql.toString(), TPipClrMertFeeCaltDO.class, start, limit, parameters);
			}
		} catch (Exception e) {
			log.error("列表查询异常", e);
	 ...
	 */
	@Override
	public List<TPipClrMertFeeCaltDO> list(int start, int limit, Object... param) {
		return null;
	}
	 ...
	 * @return total
	 */
	public int getTotal(TPipClrMertFeeCaltDO obj) {
		log.debug("TPipClrMertFeeCaltDO=" + obj);
		List<Object> parameters = Lists.newArrayList();
        StringBuilder sql = new StringBuilder();
        sql.append("select * from ").append(TABLE_NAME).append(getWhereSql(obj, parameters));
	 ...
	 * @return SQL
	 */
	public String getWhereSql(TPipClrMertFeeCaltDO obj, List<Object> parameters) {
