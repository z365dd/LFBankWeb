...
$(function(){
	...
	$('#tab2').click(function(){
		$('#tab2').text('组件新增');
		SAVE_OR_REV_OR_DETAIL = "SAVE";
		ifr('panel2','prod/oper/prodAttr/prodAttrForm',1000);
	});
...
/*详细*/
function Detail(compNo,compName,BELONG_FLG,OPEN_STAT){
	getDetail(compNo,compName,BELONG_FLG,"DETAIL",OPEN_STAT);
} 

/*修改*/
function Revice(compNo,compName,BELONG_FLG,OPEN_STAT){
	getDetail(compNo,compName,BELONG_FLG,"REV",OPEN_STAT);
}

/*获取详细数据,打开tab2传入数据*/
function getDetail(compNo,compName,BELONG_FLG,tp,OPEN_STAT){
	BODY.OPEN_STAT = OPEN_STAT;
	BODY.COMP_NO = compNo;
	BODY.COMP_NAME = compName;
	BODY.PROD_COMP_TP = BELONG_FLG;
	...
	$("#panel2").find('iframe').on("load",function(){
		$("#panel2").find('iframe')[0].contentWindow.setData(compNo,compName,BELONG_FLG,OPEN_STAT);
	});
}
...
function Delete(compNo){
	...
	$.ajax({
		url:ctx+"/prod/oper/prodAttr/delete",
		type:"POST",
		dataType:"json",
		data:{
			COMP_NO:compNo, 
...
function UpdateStat (compNo,stat){
	Ewin.confirm({
		title : "操作提示",
		message : "是否修改状态？"
	}).on(function(e) {
	...
	$.post(ctx+"/prod/oper/prodAttr/updateStat",{COMP_NO:compNo,OPEN_STAT:stat},function(data){
		if (data.returnCode !== undefined
				&& "0000" != data.returnCode) {
			var errMsg = "错误信息[" + data.message + "]";
			showContent(errMsg, "error");
