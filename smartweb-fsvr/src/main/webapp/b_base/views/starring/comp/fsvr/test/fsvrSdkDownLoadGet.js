console.log('fsvrSdkDownLoadGet.js');
...
function downLoadGetSend(){
	...
	if(proof()){
		...
		$.get(ctx+"/comp/fsvr/testSdk/pullAndDownload",{
			...
			FILE_MNG_FLG:getS("FILE_MNG_FLG"),
			...
			if(data.returnCode!==undefined && "0000"!=data.returnCode){ 
				...
				showContent(errMsg,"error");	
				 ...
				 if(data.returnCode!==undefined && "0000"!=data.returnCode){ 
						var errMsg = "文件上传并推送["+data.message+"]"; 
						showContent(errMsg,"error");	
				 }else{
					 	var dataRes = data.dataSetResult[0];
						 
					 	var successMsg = "文件获取并下载成功"; 
						showContent(successMsg,"success");
						var dataVal = dataRes.data;
						setI("REQ_SEQ",dataVal[0].seqNo);
						disabledI("REQ_SEQ");
						//设置文件请求流水号
						setI("FILE_SET_SEQ",dataVal[0].fileSeqSet);
						disabledI("FILE_SET_SEQ");
						 
						var FILE_TRANS_TP = getS("FILE_TRANS_TP");
						if ( "01" == FILE_TRANS_TP ) {
							var $fileList = dataVal;
							for(var i=0;i<$fileList.length;i++){
								$($("#fileDiv").find("a[name=downLoadBtnFile]")[i]).attr("style", "display:block");
								$($("#fileDiv").find("a[name=downLoadBtnFile]")[i]).attr("href",ctx+"/comp/fsvr/test/downLoad?file="+encodeURIComponent($fileList[i].fileName));
							...
							for(var a=0;a<$('[name="FILE_NAME"]').length-1;a++){
								fileObjArr[a] = new fileObj($('[name="FILE_NAME"]').eq(a+1).val(),$('[name="ALIAS_FILE_NAME"]').eq(a+1).val(),$('[name="SUB_FILE_PATH"]').eq(a+1).val());
								$($("#fileDiv").find("a[name=downLoadBtnFile]")[a]).attr("style", "display:block");
								$($("#fileDiv").find("a[name=downLoadBtnFile]")[a]).attr("href",ctx+"/comp/fsvr/test/downLoad?file="+encodeURIComponent(fileObjArr[a].FILE_NAME));
							}
						}
					}
			}
