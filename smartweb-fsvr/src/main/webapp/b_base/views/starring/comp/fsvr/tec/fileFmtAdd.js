console.log('fileFmtAdd.js');
var arr = []; // 数据暂存于此
var headSer = 0;// 报文头数，当前报文头最大序号
var bodySer = 0;// 报文体数
var endSer = 0;// 报文尾数
var chgNum = 0;// 枚举转换数量
...
$(document)
		.ready(
				function() {
					...
					/*
					 * 格式类型控制输入框的显隐， 格式类型为定长（1）时显示字段长度和对齐方式，隐藏分隔符
					...
					/*
					 * $("#upBtn").click(function() { moveUp(); });
					 * 
					 * $("#downBtn").click(function() { moveDown(); });
					 */
...
function tableActionRow(flg, tp) {
	...
	if (tp == "add") {
		...
		if (rev_index == -1) {
			...
			/*
			 * Data.ser = arr[rev_index].ser; arr.splice(rev_index, 1,
			 * rowData);// 替换掉数组中对应index 的data arr[rev_index].ser = Data.ser;
			 */
...
function delRow(obj) {
	...
	// 修改arr里索引后面所有数据的ser
	/*
	 * for (var i = index; i < arr.length; i++) { if (position == arr[i].flg) {
	 * arr[i].ser = arr[i].ser - 1; } else { break; } }
	 */
	// ser = ser - 1;
	// 刷新一下表格
...
function save() {
	...
	if (formData == null) {
		...
		/* 向后台发送参数 */
		$.post(ctx + "/comp/fsvr/tec/fsvrFileFmt/insert", formData, function(
				data) {
...
function bigColChg(flg) {
	if (getS('colChgFlg_' + flg) == "1") {
		...
		for (var i = 0; i < row.length; i++) {
			if (row[i].tempFlg == tempFlg
					&& row[i].tempSer.substr(0, 1) == tempSer) {
			...
			.post(
					...
					function(data) {
						if (data.returnCode !== undefined
							...
							$('#table_head').bootstrapTable('removeAll');
							$('#table_body').bootstrapTable('removeAll');
							headSer = 0;
							bodySer = 0;
							endSer = 0;
							cleanTabDiv('head');
...
function getTranTp(tempFmtNo) {
	$.post(ctx + "/comp/fsvr/tec/tfsvrFileTotPara/get", {
		fmtNo : tempFmtNo
	}, function(data) {
		if (data.returnCode !== undefined && "0000" != data.returnCode) {
			var errMsg = "错误信息[" + data.message + "]";
