...
$(function (){
	...
	function cue(self,nameId){
		...
		if($(self).attr("check-edition") == "true" ){
			var tipVal = '请输入类似1.0或1.0.1的版本格式,主版本号不能为0,长度不能大于10'
			greyHints(tipVal,nameId);
		};
		/* 验证英文 金额币种 */
 ...
 * 动态加载
 * */
function dynamicDom(id) {
	/*console.log(id+"开始校验")*/
	var bodyDom = id != undefined ?"#"+id:"body";
	$(bodyDom).on("blur","input",function(){
		 console.log(this);/*控制单个 */
		var inputLength = id != undefined? $("#"+id+" input[type='text'],textarea:visible"):$("input[type='text'],textarea:visible");
		for (var i=0; i< inputLength.length; i++) {
			/*console.log(inputLength[i]);*/
				/* 获取点击input的id */
			var nameId  = $(inputLength[i]).attr("id");
			if (nameId == undefined) {
				continue;
			}
			var inputVal = $("#"+nameId).val();
			var _val = inputVal.length;
			/*最大长度*/
			var _maxlength = $("#"+nameId).attr("maxlength");
			/*最小长度*/
			var _min = $("#"+nameId).attr("check-minlength");
			/* 失去焦点删除灰色提示 */
			$("#"+nameId).parent().siblings('span.promptScript').remove();
			var $self =  $("#"+nameId);
			var tipVal;
            inputcheck($self)
				if ($self.attr("disabled") != "disabled") {
