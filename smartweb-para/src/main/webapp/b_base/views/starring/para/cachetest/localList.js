$(document).ready(function(){
	...
	$("#listBtn").click(function(){
		if (chkExpr()) {
			...
			queryList();
			$("#coltable").bootstrapTable("removeAll");
			$("#uniqueKey").multiselect("select", "").multiselect("rebuild");
			$("#uniqueValue").val("");
		}
	});
	...
	chgOperSelect(custom);
	$("#custom").on("change", function () {
		chgOperSelect($(this).val());
	});
});
function chgOperSelect(custom) {
	if (custom == "AND") {
		$("#oper").multiselect("select", "=").multiselect("rebuild");
		$("#oper").multiselect("disable");
	} else if (custom == "OR") {
		$("#oper").multiselect("rebuild");
	}
}
function queryList() {
	...
	genData();
	var tableData = "";
	// ${ctx}/cache/chk/data/dataList
	$("#rules").multiselect("rebuild");
	$("#custom").multiselect("rebuild");
	$("#oper").multiselect("rebuild");
	var formData = $("#listForm").serializeObject();
	$.post(ctx + "/para/test/local/list", {
		rules: (typeof(formData.rules)==undefined)?'':formData.rules,
		custom: (typeof(formData.custom)==undefined)?'':formData.custom,
		colJson: (typeof(formData.colJson)==undefined)?'':formData.colJson,
