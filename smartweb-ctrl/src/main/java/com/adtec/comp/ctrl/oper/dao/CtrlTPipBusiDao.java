 ...
 */
@Component
public class CtrlTPipBusiDao implements IBaseDao<CtrlTPipBusiDO>{
	/**
	 * 日志对象
	 */
	protected final static Logger log = LoggerFactory.getLogger(CtrlTPipBusiDao.class);
	/*表名称*/
	 ...
	 * @return
	 */
	public CtrlTPipBusiDO get(String busiNo ) {
		CtrlTPipBusiDO obj = new CtrlTPipBusiDO();
	 ...
	 */
	@Override
	public int insert(CtrlTPipBusiDO obj){
		int rs = 0;
		IDBSession session = DBSessionFactory.getSession("fctrl");
		try {
	 ...
	 */
	@Override
	public int update(CtrlTPipBusiDO obj){
		int rs = 0;
		IDBSession session = DBSessionFactory.getSession("fctrl");
		try {
			List<String> ignoreFields = obj.getIgnoreFields();
	 ...
	 */
	@Override
	public int delete(CtrlTPipBusiDO obj){
		int rs = 0;
		IDBSession session = DBSessionFactory.getSession("fctrl");
		try {
			List<Object> parameters = Lists.newArrayList();
			String sql = "DELETE FROM t_pip_busi "+getWhereSql(obj, parameters);
	 ...
	 */
	@Override
	public List<CtrlTPipBusiDO> list(CtrlTPipBusiDO obj) {
	 ...
	 */
	@Override
	public List<CtrlTPipBusiDO> list(CtrlTPipBusiDO obj, int start, int limit) {
		log.debug("CtrlTPipBusiDO=" + obj.toString());
		log.debug("start=" + start);
		log.debug("limit=" + limit);
		List<CtrlTPipBusiDO> list = null;
		try {
			List<Object> parameters = Lists.newArrayList();
			StringBuilder sql = new StringBuilder();
        	sql.append("select * from ").append(TABLE_NAME).append(getWhereSql(obj, parameters));
        	...
        	IDBSession session = DBSessionFactory.getSession("fctrl");
			if (limit <= 0) {
				list = session.getObjectListByList(sql.toString(), CtrlTPipBusiDO.class, parameters);
			} else {
				list = session.getObjectListByListForPage(sql.toString(), CtrlTPipBusiDO.class, start, limit, parameters);
			}
		} catch (Exception e) {
			log.error("列表查询异常", e);
	 ...
	 */
	@Override
	public List<CtrlTPipBusiDO> list(int start, int limit, Object... param) {
		return null;
	}
	 ...
	 * @return total
	 */
	public int getTotal(CtrlTPipBusiDO obj) {
		log.debug("CtrlTPipBusiDO=" + obj);
		List<Object> parameters = Lists.newArrayList();
        StringBuilder sql = new StringBuilder();
        sql.append("select * from ").append(TABLE_NAME).append(getWhereSql(obj, parameters));
        ...
        try {
        	...
        	log.error("总记录数查询t_pip_busi异常", e);
	 ...
	 * @return SQL
	 */
	public String getWhereSql(CtrlTPipBusiDO obj, List<Object> parameters) {
		if(null==parameters){
