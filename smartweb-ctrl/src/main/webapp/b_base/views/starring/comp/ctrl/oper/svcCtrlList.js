...
$(function(){
	...
	//setSelect2("COMP_NO", "/comp/ctrl/oper/svccode/getModlNo","COMP_NO","COMP_NAME", {Flg:"All"}, "All", null,true);
	setCompNoS("COMP_NO",true,true);
	
	/*模型下拉改变则重新加载服务码下拉框*/
	setSvcCodeS("COMP_NO","SVC_CODE",true);
	//svcCodeS("COMP_NO","SVC_CODE");
	...
	$('#tab2').click(function(){
		$('#tab2').text('服务码控制新增');
		SAVE_OR_REV = "add";
		ifr('panel2','comp/ctrl/oper/svcCtrl/svcCtrlForm',600);
	});
});
...
/*修改*/
function Revice(compNo,svcCode,stat){
	$('#tab2').click();
	$('#tab2').text('服务码控制修改');
	SAVE_OR_REV = "revice";
	$("#panel2").find('iframe').load(function() {
		$("#panel2").find('iframe')[0].contentWindow.setData(compNo,svcCode,stat);
	});
}
...
function Delete(compNo,svcCode){
	...
	$.ajax({
		url:ctx+"/comp/ctrl/oper/svcCtrl/action", 
		type:"POST",
		dataType:"json",
		data:{
			COMP_NO:compNo, 
