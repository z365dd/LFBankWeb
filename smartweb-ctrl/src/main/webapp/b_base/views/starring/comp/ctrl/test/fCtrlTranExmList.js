...
/*//table 新增一行(公用维度)
function pubDimTableAddRow(tableId){
	var index = $('#'+tableId).bootstrapTable('getData').length;
	var PUB_DIM_KEY = getS('PUB_DIM_KEY');
	var Date={
			PUB_DIM_KEY:PUB_DIM_KEY,
			PUB_DIM_KV:getI('PUB_DIM_KV')
	}
...
function saveBtnFun(){
	if(portion("pubSel")){
		...
		if(caseName!=""){
			...
			$.ajax({
				type:"POST",
				url:ctx + "/comp/ctrl/test/tranctrlchk/saveExm",
				dataType:"json",
				data:{
					caseNo:caseNo,
					caseName:caseName,
...
function saveBtnFun1(){
	if(portion("pubSel")){
		...
		parent.showTabs("#tab_save", ctx+"/comp/ctrl/test/tranctrlchk/toSave");
...
function delBtnFun(){
	...
	if(caseName!=""){
		...
		Ewin.confirm({
			...
			$.ajax({
				type:"POST",
				url:ctx + "/comp/ctrl/test/tranctrlchk/delExm",
				dataType:"json",
				data:{
					caseName:caseName,
					caseNo:caseNo
...
function testBtnFun(){
	if(portion("pubSel")){
		...
		$.ajax({
			type:"POST",
			url:ctx + "/comp/ctrl/test/tranctrlchk/testExm",
			dataType:"json",
			data:{
				COMP_NO:COMP_NO,
				SVC_CODE:SVC_CODE,
