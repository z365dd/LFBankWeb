 ...
 */
@Component
public class TseqTParaRelatSysDao implements IBaseDao<TseqTParaRelatSysDO>{
	/**
	 * 日志对象
	 */
	protected final static Logger log = LoggerFactory.getLogger(TseqTParaRelatSysDao.class);
	/*表名称*/
	 ...
	 * @return
	 */
	public TseqTParaRelatSysDO get(String relatSys ) {
		TseqTParaRelatSysDO obj = new TseqTParaRelatSysDO();
	 ...
	 */
	@Override
    public TseqTParaRelatSysDO get(TseqTParaRelatSysDO obj) {
        StringBuilder sql = new StringBuilder();
        List<Object> parameters = Lists.newArrayList();
        sql.append("select * from ").append(TABLE_NAME).append(getWhereSql(obj, parameters));
        TseqTParaRelatSysDO rs = null;
        IDBSession session = DBSessionFactory.getSession();
        try {
            rs = session.getObjectByList(sql.toString(), TseqTParaRelatSysDO.class, parameters);
        } catch (Exception e) {
	 ...
	 */
	@Override
	public int insert(TseqTParaRelatSysDO obj){
		int rs = 0;
		IDBSession session = DBSessionFactory.getSession();
		try {
	 ...
	 */
	@Override
	public int update(TseqTParaRelatSysDO obj){
		int rs = 0;
		IDBSession session = DBSessionFactory.getSession();
		try {
			List<String> ignoreFields = obj.getIgnoreFields();
	 ...
	 */
	@Override
	public int delete(TseqTParaRelatSysDO obj){
		int rs = 0;
		IDBSession session = DBSessionFactory.getSession();
		try {
			List<Object> parameters = Lists.newArrayList();
	 ...
	 */
	@Override
	public List<TseqTParaRelatSysDO> list(TseqTParaRelatSysDO obj) {
	 ...
	 */
	@Override
	public List<TseqTParaRelatSysDO> list(TseqTParaRelatSysDO obj, int start, int limit) {
		log.debug("TseqTParaRelatSysDO=" + obj.toString());
		log.debug("start=" + start);
		log.debug("limit=" + limit);
		List<TseqTParaRelatSysDO> list = null;
		try {
			List<Object> parameters = Lists.newArrayList();
			StringBuilder sql = new StringBuilder();
        	sql.append("select * from ").append(TABLE_NAME).append(getWhereSql(obj, parameters));
        	...
        	IDBSession session = DBSessionFactory.getSession();
			if (limit <= 0) {
				list = session.getObjectListByList(sql.toString(), TseqTParaRelatSysDO.class, parameters);
			} else {
				list = session.getObjectListByListForPage(sql.toString(), TseqTParaRelatSysDO.class, start, limit, parameters);
			}
		} catch (Exception e) {
			log.error("列表查询异常", e);
	 ...
	 */
	@Override
	public List<TseqTParaRelatSysDO> list(int start, int limit, Object... param) {
		return null;
	}
	 ...
	 * @return total
	 */
	public int getTotal(TseqTParaRelatSysDO obj) {
		log.debug("TseqTParaRelatSysDO=" + obj);
		List<Object> parameters = Lists.newArrayList();
        StringBuilder sql = new StringBuilder();
        sql.append("select * from ").append(TABLE_NAME).append(getWhereSql(obj, parameters));
	 ...
	 * @return SQL
	 */
	public String getWhereSql(TseqTParaRelatSysDO obj, List<Object> parameters) {
		if(null==parameters){
