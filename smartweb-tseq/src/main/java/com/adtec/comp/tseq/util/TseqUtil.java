...
public class TseqUtil {
	...
	public static void setReqHead(String busiNo,ReqDTO req,String svcCode, int start,int pageSize){
		...
		String modelNo = source.getString("FLOW_ModelNo");*/
		String modelNo = ParamUtil.getConfig("FLOW_PARTID");
		req.getSYS_HEAD().setREQ_MODL_NO(modelNo);	//请求组件/模型编号
		String seqNo = SeqUtil.getMBCSeq();
		req.getSYS_HEAD().setREQ_SEQ_NO(DateUtils.getDate() + seqNo); // 服务请求发送方流水号
		req.getSYS_HEAD().setSEQ_NO(DateUtils.getDate() + seqNo);//内部全局流水号
		req.getSYS_HEAD().setREQ_DATE(DateUtils.getDate());
		/*修改*/
		req.getSYS_HEAD().setREQ_COMP_NO(modelNo);	//请求组件/模型编号
		req.getSYS_HEAD().setREQ_SEQ(DateUtils.getDate() + seqNo); // 服务请求发送方流水号
		req.getSYS_HEAD().setSND_SEQ(DateUtils.getDate() + seqNo);//内部全局流水号
		...
		req.getAPP_HEAD().setTX_DATE(DateUtils.getDate());//交易日期
		req.getAPP_HEAD().setTX_TIME(DateUtils.getTime());//交易时间
		/*修改*/
		req.getAPP_HEAD().setTRAN_DATE(DateUtils.getDate());//交易日期
		req.getAPP_HEAD().setTRAN_TIME(DateUtils.getTime());//交易时间
		...
		req.getLOCAL_HEAD().setCHNL_SEQ_NO(DateUtils.getDate() + seqNo);
