...
function queryParams(params){
    ...
    var paramList = {
		outSys: (typeof(formData.outSys)==undefined)?'':formData.outSys,
		outSubSys: (typeof(formData.outSubSys)==undefined)?'':formData.outSubSys,
...
/*跳转更新页面*/
function update(seqCrtId){
	console.info("open Update tab");
	var para = "";
	para = para + "?";
	$.session.set('HID_seqCrtId',seqCrtId);
	para = para + "seqCrtId="+seqCrtId;
	parent.window.$("a[href^='#tab_update']").attr("url", ctx + "/comp/tseq/oper/tTseqSeqCrt/tTseqSeqCrtUpdate" + para);
...
function detail(seqCrtId){
	...
	$.session.set('HID_seqCrtId',seqCrtId);
	para = para + "seqCrtId="+seqCrtId;
	parent.window.$("a[href^='#tab_detail']").attr("url", ctx + "/comp/tseq/oper/tTseqSeqCrt/tTseqSeqCrtDetail" + para);
