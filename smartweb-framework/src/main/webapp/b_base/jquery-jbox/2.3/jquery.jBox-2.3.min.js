(function(a) {
	a.jBox = function(b, c) {
		...
		if (c.showFade) {
			...
			fade(a("iframe"), function(obj, ifr) {
						ifr.find("body")
								.append(iframeA + "jbox-fade" + iframeB)
					})
		} else {
			fade(a("body, iframe"), function(obj, ifr) {
						ifr.find("body").append(iframeA + "jbox-fade2"
								+ iframeB)
					})
		}
		j
		...
		a.each(b, function(N, O) {
			...
			r
					.push('<div style="min-width:50px;width:'
							...
							+ Q + ';overflow:hidden;overflow-y:auto;">'
							+ O.content + "</div></div>");
			r
					.push('<div class="jbox-button-panel" style="height:25px;padding:5px 0 5px 0;text-align: center;'
							+ (P ? "" : "display:none;") + '">');
			if (!c.isTip) {
				r
			...
			a.each(O.buttons, function(T, U) {
				...
				if(U=="ok"){
					...
					css="jbox-button-clear"
				}else if(U==true||U=="cancel"){
					css="jbox-button-close"
				}else{
					css="jbox-button-ok"
				}
						r.push('<button class="'+css+'" value="' + U
								+ '" style="' + u + '">' + T + "</button>")
					});
			r.push("</div></div>")
		});
		...
		a.each(b, function(N, O) {
					...
					P.children(".jbox-button-panel").children("button").click(
							function() {
								...
								var S = {};
								a.each(	o.find("#jbox-states :input")
												.serializeArray(), function(U,
												V) {
		...
		var w = function() {
			var N = l.width();
			return document.body.clientWidth < N
					? N
				...
				if (c.isTip) {
					...
					if (tip && tip.next == true) {
						...
						if (N == o.offset().top) {
							...
							o.find("#jbox-content").html(tip.options.content
									.substr(5)).end().css({
										left : ((l.width() - o.outerWidth()) / 2)
									}).animate({
										top : N,
		...
		var J = function() {
			...
			if (c.showFade) {
				p.fadeOut("fast", function() {
							p.unbind("click", y).unbind("mousedown", C)
									.html("").remove()
						})
			}
			...
			if (c.timeout > 0) {
				...
				if (c.isMessager) {
					o.hover(function() {
								...
								o.data("autoClosing", window.setTimeout(I,
												c.timeout))
							})
				}
			}
			...
			switch (f.type) {
				...
				case "IFRAME" :
					o.find("#jbox-iframe").attr({
						...
						o
								.find("#jbox-states .jbox-state:first .jbox-button-focus")
								.focus();
						L()
					});
					...
					if (c.ajaxData != undefined) {
						for (var x in c.ajaxData) {
							jboxIframeForm.append('<input type="hidden" name="'
									+ x + '" value="' + c.ajaxData[x] + '"/>')
						}
					}
		...
		if (!c.isTip) {
			...
			o.find(".jbox-container,.jbox-content").addClass("jbox-tip-color-"+c.icon);
		}
		if (f.type != "IFRAME") {
	...
	a.jBox.goToState = function(b, c) {
		...
		if (d != undefined && d != null) {
			...
			if (typeof b == "string") {
				...
				e = b ? d.find(".jbox-state:visible").next() : d
						.find(".jbox-state:visible").prev()
			}
	...
	a.jBox.prompt = function(b, c, d, e) {
		...
		jQuery.browser={};(function(){jQuery.browser.msie=false; jQuery.browser.version=0;if(navigator.userAgent.match(/MSIE ([0-9] )./)){ jQuery.browser.msie=true;jQuery.browser.version=RegExp.$1;}})();
		var g = e.icon == "none" ? "height:auto;" : "min-height:30px;"
				+ ((a.browser.msie && parseInt(a.browser.version) < 7)
	...
	a.jBox.messager = function(b, c, d, e) {
		...
		jQuery.browser={};(function(){jQuery.browser.msie=false; jQuery.browser.version=0;if(navigator.userAgent.match(/MSIE ([0-9] )./)){ jQuery.browser.msie=true;jQuery.browser.version=RegExp.$1;}})();
		var h = e.icon == "none" ? "height:auto;" : "min-height:30px;"
				+ ((a.browser.msie && parseInt(a.browser.version) < 7)
						? "height:auto !important;height:100%;_height:30px;"
						: "height:auto;");
		var i = [];
