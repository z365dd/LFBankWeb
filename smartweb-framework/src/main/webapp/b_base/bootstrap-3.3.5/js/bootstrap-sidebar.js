/* ========================================================================
 * Bootstrap: bootstrap sidebar.js v3.3.5
 * ========================================================================
 * Copyright 2016-2016 pangzhe, Inc.
...
ace.submenu = {
    ...
    show: function(e, a) {
        ...
        //pangzhe 
        var b = s.siblings('a').children('b');
        b.removeClass('glyphicon-menu-right');
        b.addClass('glyphicon-menu-down');
        //end
        s.css({
            height: 0,
            overflow: "hidden",
    ...
    hide: function(e, a) {
        ...
        //pangzhe 
        var b = s.siblings('a').children('b');
        b.removeClass('glyphicon-menu-down');
        b.addClass('glyphicon-menu-right');
        //end
        s.css({
            height: e.scrollHeight,
            overflow: "hidden"
...
ace.settings = {
    ...
    sidebar_collapsed: function(e, a) {
        ...
        if(e){
        	/*
        	$(t).parent('.column').removeClass('col-md-2');
        	$(t).parent('.column').addClass('col-md-1');
        	$(t).parent('.column').next().removeClass('col-md-10');
        	$(t).parent('.column').next().addClass('col-md-11');
        	$(t).parent('.column').next().css('margin-left','-70px');*/
        	/*$(t).css('overflow-y','');*/
        	$('#sidebarleft').css({
        	    'width':'45px',
        	    'overflow-y':'visible',
        	...
        	$('#sidebarright').css('padding-left','45px');
        	...
        	$('#sidebarright').css('padding-left','240px');
        	/*$(t).css('overflow-y','auto');
        	$(t).parent('.column').removeClass('col-md-1');
        	$(t).parent('.column').addClass('col-md-2');
        	$(t).parent('.column').next().removeClass('col-md-11');
        	$(t).parent('.column').next().addClass('col-md-10');
        	$(t).parent('.column').next().css('margin-left','0px');*/
        }
...
/*--------bootstrap sidebar 侧边栏------------*/
//TODO
+function ($) {
    'use strict';//1.0使用严格模式ES5支持
	...
	var Sidebar = function (element, options) {
		  ...
		  $('#sidebar').on('click','a:not(.dropdown-toggle)',function () {
			  ...
			  var $this = $(this);
			  $this.parentsUntil('.sidebar','li').addClass('active');
			  
			  if(options.onClick && typeof options.onClick ==='function') {
					var url = $(this).attr('url');
					var name = $(this).children('span').text();
					/*var ret = options.onClick(url,name);*/
					var data = $(this).parent('li').data('data');
					var ret = options.onClick(url,name,data);
					if(ret===false) return;
			  }
		  });
		  
		  	//是否显示缩放按钮
		  /*if(this.options.showCollapsible){
			  var _x =   '<div class="sidebar-toggle sidebar-collapse" id="sidebar-collapse">'+
			      		'<i class="ace-icon glyphicon glyphicon-triangle-left" data-icon1="ace-icon glyphicon glyphicon-triangle-left" data-icon2="ace-icon glyphicon glyphicon-triangle-right"></i>'+
			        '</div>'
      		  this.$element.append(_x);
		  }*/
		  ...
		  if(this.options.async_enable){
			  $.ajax({ 
				  type: options.async_type,
					url:options.async_url,
					dataType : "json",
					data:options.otherParam,
	...
	Sidebar.DEFAULTS = {
			async_enable:false,//设置 zTree 是否开启异步加载模式 默认值：false
	    	async_type:'post',//Ajax 的 http 请求模式。[setting.async.enable = true 时生效] 默认值："post"
	    	...
	    	showCollapsible:true,//是否显示折叠按钮 默认显示true
	}
	
	
	
	Sidebar.prototype.init= function (relatedTarget) {
		// 第一层菜单
		var $element = this.$element;
		var options = this.options;
		if($element.children('ul').length == 0){
			$element.prepend('<ul style="top: 0px;" class="nav nav-list" ></ul>');
		}
		
		var name = options.name;
		var pId = options.pIdKey;
		var id = options.idKey;
		
		var $ul = $element.children('ul');
		var i=0;
		$(relatedTarget).each(function (index) {
			if(this[pId]==null || this[pId] =='' || this[pId] ==undefined){
				...
				var _x0 = 
					...
					'<a url = "'+this.url+'">'+
					//TODO
					'<i class="menu-icon '+'glyphicon glyphicon-th-list'+'" aria-hidden="true"></i>'+
					/*'<i class="menu-icon '+this.imgUrl+'" aria-hidden="true"></i>'+*/
					'<span class="menu-text">'+this[name];
				var _x1 = 
					'</span>'+
					'</a>'+
					'<b class="arrow">'+
					'</b>'+
					'</li>';
				if(this.badge == undefined || this.padge == ''  || this.padge =='undefined'){
					x=_x0+_x1;
				}else {
					x = _x0+'<span class="badge badge-primary"> '+this.badge+' </span>'+_x1;
				}
				$ul.append(x);
				$ul.find('#'+this[id]).data('data',this);
				relatedTarget.splice(index-i,1);
				i++;
			}
		});
		//第二层菜单
		...
		$(relatedTarget).each(function (index) {
			...
			if(this[pId]!=null && this[pId] !='' && this[pId] !=undefined){
				var pdom = $element.find('#'+this[pId]);
				if(pdom.length==1){
					pdom.children().first().addClass('dropdown-toggle');
					pdom.children().first().append('<b class="arrow glyphicon glyphicon glyphicon-menu-right"></b>');
					if(!pdom.children('ul').is('ul')){
						pdom.append('<ul class="submenu"> </ul>');
					}
					var xml = 
							'<li id = '+this[id]+'>'+
								'<a url = "'+this.url+'">'+
						            '<i class="menu-icon glyphicon glyphicon-triangle-right"></i>'+
						            this[name]+
						        '</a>'+
						        '<b class="arrow">'+
						        '</b>'+
						    '</li>';
					pdom.children('ul').append(xml);
					pdom.find('#'+this[id]).data('data',this);
	...
	Sidebar.prototype.add= function (relatedTarget) {
	...
	function Plugin(option,_relatedTarget) {
	    ...
	    this.each(function () {
		    ...
		    var data  = $this.data('bs.sidebar')//取缓存tabs对象
		    var options = $.extend({}, Sidebar.DEFAULTS, $this.data(), typeof option == 'object' && option)//合并配置参数
		    if(!data){
