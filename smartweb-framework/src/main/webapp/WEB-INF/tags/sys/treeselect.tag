...
<%@ attribute name="not_allow_select_child" type="java.lang.Boolean" required="false" description="不允许选择子节点"%>
...
<div class="input-append col-sm-6">
	...
	<input input_type="treesearch" search_title="${title}" placeholder="请选择${title }" search_url="${url}" id="${id}Name" name="${label_name}" ${allow_input?'':'readonly="readonly"'} type="text" value="${label_value}" data-msg-required="${data_msg_required}" 
		class="${css_class}" style="${css_style}" checked="${checked}" ext_id="${ext_id}" is_all="${is_all}" not_allow_select_root="${not_allow_select_root}" not_allow_select_parent="${not_allow_select_parent}" not_allow_select_child="${not_allow_select_child}" 
			...
			ajaxData:{selectIds: $("#${id}Id").val()},buttons:{"确定":"ok", ${allow_clear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				...
				if (v=="ok"){
					...
					for(var i=0; i<nodes.length; i++) {
						...
						if (nodes[i].level != 0  && $("#${id}Name").attr("not_allow_select_child")=="true"){
							/* 不允许选择父节点 */
							top.$.jBox.tip("不能选择子节点（"+nodes[i].name+"）请重新选择。","error");
							return false;
						}
