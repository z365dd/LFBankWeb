...
<ul class="yzm">
  <li class="yzm-left"><input class="login-yzm required" type="text" id="${name}" name="${name}" /></li>
  <li class="yzm-right"><a href="javascript:" onclick="$('.${name}').attr('src','${pageContext.request.contextPath}/servlet/validateCodeServlet?'+new Date().getTime());" class="mid ${name}Refresh" ><img src="${pageContext.request.contextPath}/servlet/validateCodeServlet" onclick="$('.${name}Refresh').click();" class="mid ${name}" /></a></li>
</ul>
<!-- 
