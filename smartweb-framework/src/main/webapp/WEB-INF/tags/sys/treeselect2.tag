...
<%@ attribute name="input_flag" type="java.lang.Boolean" required="false" description="是否显示输入框"%>
<div class="input-append col-sm-6">
	...
	<input input_type="treesearch" search_title="${title}"  placeholder="请选择${title }" search_url="${url}" id="${id}Name" name="${label_name}" ${allow_input?'':'readonly="readonly"'} <c:if test="${input_flag}">type="hidden"</c:if> value="${label_value}" data-msg-required="${data_msg_required}" 
