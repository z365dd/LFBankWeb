...
<head>
	...
	<script type="text/javascript" charset="utf-8">
        $(document).ready(function(){
            $("#legaName").val("${corporationVoOwn.name}");
            $("#legaName").attr("disabled","disabled");
            ...
            // 机构-菜单
            var zNodes=[
                    <c:forEach items="${corporationVoOwn.parentMenuList}" var="menu">{id:"${menu.id}", pId:"${not empty menu.parent.id?menu.parent.id:0}", name:"${not empty menu.parent.id?menu.name:'权限列表'}"},
                </c:forEach>];
            // 初始化树结构
            ...
            // 默认选择节点
            var ids = "${corporationVoOwn.menuIds}".split(",");
			...
			// 机构-菜单
			var zNodes=[
					<c:forEach items="${corporationVoTransfer.parentMenuList}" var="menu">{id:"${menu.id}", pId:"${not empty menu.parent.id?menu.parent.id:0}", name:"${not empty menu.parent.id?menu.name:'权限列表'}"},
				</c:forEach>];
			// 初始化树结构
			...
			// 默认选择节点
			var ids = "${corporationVoTransfer.menuIds}".split(",");
			for(var i=0; i<ids.length; i++) {
				var node = tree.getNodeByParam("id", ids[i]);
				try{tree.checkNode(node, true, false);}catch(e){}
			}
			...
			// 机构-菜单
			var zNodes=[
					<c:forEach items="${corporationVoUse.parentMenuList}" var="menu">{id:"${menu.id}", pId:"${not empty menu.parent.id?menu.parent.id:0}", name:"${not empty menu.parent.id?menu.name:'权限列表'}"},
				</c:forEach>];
			// 初始化树结构
			...
			// 默认选择节点
			var ids = "${corporationVoUse.menuIds}".split(",");
			for(var i=0; i<ids.length; i++) {
				var node = tree.getNodeByParam("id", ids[i]);
				try{tree.checkNode(node, true, false);}catch(e){}
			}
...
<ul class="nav nav-tabs">
		...
		<shiro:hasPermission name="sys:corporation:edit"><li class="active"><a href="${ctx}/sys/corporation/seePermissionToCorporation?id=${corporationVoOwn.id}">查看权限组</a></li></shiro:hasPermission>
		</c:if>
</ul>
<form:form id="inputForm" modelAttribute="corporationVoOwn" action="" method="post" class="form-horizontal">
