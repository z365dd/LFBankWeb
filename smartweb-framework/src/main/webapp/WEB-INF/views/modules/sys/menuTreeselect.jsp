...
<script type="text/javascript"> 
	...
	var menuTreeselectNodes=[
			<c:forEach items="${fns:getMenuList()}" var="menu"><c:if test="${menu.dpyFlg eq '1'}">{id:"${menu.id}", pId:"${not empty menu.parent.id ? menu.parent.id : 0}", name:"<i class=\"icon-${not empty menu.appIcon ? menu.appIcon : 'file'}\"></i>&nbsp;${not empty menu.parent.id ? menu.name : ''}", url:"${not empty menu.menuLink && fn:indexOf(menu.menuLink, '://') eq -1 ? ctx : ''}${not empty menu.menuLink ? menu.menuLink : 'javascript:'}", target:"${not empty menu.windowVal ? menu.windowVal:'_self'}"},
