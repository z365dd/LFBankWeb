...
<body>
	...
	<form:form id="searchForm" modelAttribute="genTable" action="${ctx}/gen/genTable/" method="post" class="breadcrumb form-search">
		...
		<label>表名：</label><div style="display:inline-block;"><form:input path="nameLike" htmlEscape="false" maxlength="50" class="input-medium"/></div>
		<label>说明：</label><div style="display:inline-block;"><form:input path="tabDesc" htmlEscape="false" maxlength="50" class="input-medium"/></div>
		<label>父表表名：</label><div style="display:inline-block;"><form:input path="parentTabName" htmlEscape="false" maxlength="50" class="input-medium"/></div>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="genTable">
			<tr>
				...
				<td>${genTable.tabDesc}</td>
				<td>${genTable.procClssName}</td>
				<td title="点击查询子表"><a href="javascript:" onclick="$('#parentTabName').val('${genTable.parentTabName}');$('#searchForm').submit();">${genTable.parentTabName}</a></td>
