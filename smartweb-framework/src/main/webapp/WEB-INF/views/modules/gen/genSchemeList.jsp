...
<body>
	...
	<form:form id="searchForm" modelAttribute="genScheme" action="${ctx}/gen/genScheme/" method="post" class="breadcrumb form-search">
		...
		<label>方案名称 ：</label><div style="display:inline-block;"><form:input path="name" htmlEscape="false" maxlength="50" class="input-medium"/></div>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="genScheme">
			<tr>
				...
				<td>${genScheme.packName}</td>
				<td>${genScheme.modlName}${not empty genScheme.subModlName?'.':''}${genScheme.subModlName}</td>
				<td>${genScheme.funcName}</td>
				<td>${genScheme.funcCrtr}</td>
