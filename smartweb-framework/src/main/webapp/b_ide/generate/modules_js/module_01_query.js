/* ========================================================================
 * Rainbow: module_01_query.js v1.0.0 
 * Author: chenyl
 * Time: 20170517
...
var m_query ;
var $query_data = undefined;

var queryButtonArrayList= new ArrayList();
var queryButtonAttrMap = new Hashtable();
...
var PrivateAttrQueryMap = new Hashtable();
 ...
 $(function () {
	 console.info('query isInitModule='+isInitModule);
	 if(isInitModule){
		 init_query();
 ...
 function init_query(){
	 ...
	 $.ajax({
		   ...
		   success: function(data){
			   ...
			   $query_data = data;
			   $('#table_query').bootstrapTable('load',data.trans.table.field);
		   }
	});
	...
	$('#query_buttons').multiselect({
		 onChange: function(option,checked,select) {
			 ...
			 if(queryButtonArrayList.size()>0){
				 var data = new Array();
				 queryButtonAttrMap.clear(); //清理按钮与交易绑定映射
				 for(var i=0;i<queryButtonArrayList.size();i++){
					 //按钮关联接口列表显示
					 var r = {
						"btnId":queryButtonArrayList.get(i),
						"btnName":getModule_01_ButtonLabel(queryButtonArrayList.get(i)),
						"interface":$query_data.trans.table.interface[0].id
					 };
					 data.push(r);
					 
					 //重新初始化页面按钮的交易绑定
					 var attribute = new Hashtable();
		 			 attribute.add('btnId',queryButtonArrayList.get(i));
		 			 attribute.add('btnName',getModule_01_ButtonLabel(queryButtonArrayList.get(i)));
		 			 attribute.add('tranId',$query_data.trans.table.interface[0].id);
		 			 attribute.add('interface', $query_data.trans.table.interface[0]);
		 			 queryButtonAttrMap.add(queryButtonArrayList.get(i),attribute);
				 }
				 console.info('data='+data);
				 $('#table_query_buttons').bootstrapTable('load',data);
			 }
	...
	//初始化按钮绑定交易table
	 $('#table_query_buttons').bootstrapTable({
		 onPostBody:function(){
			 def_button_attr_query();
		 },
		 onEditableSave:function(field, row, oldValue, $el){
			 $('#table_query_buttons').bootstrapTable('resetView', {
	         });
		 },
		 onExpandRow:function(index, row, $detail){
 
			 var attr = queryButtonAttrMap.get(row.btnId);
			 
			 var ifcId = attr.interface;

			 if(ifcId==='query'){/*查询按钮请求与响应报文映射配置*/
				 onExpand_interface(index, row, $detail,'queryBtn');
			 }else if(ifcId==='update'){/*修改按钮请求与响应报文映射配置*/
				 onExpand_interface(index, row, $detail,'updateBtn');
			 }else if(ifcId==='delete'){/*删除按钮请求与响应报文映射配置*/
				 onExpand_interface(index, row, $detail,'deleteBtn');
			 }
	 }
	 });
	//table 初始化
 ...
 /*--------------2.0 formate datagrid attr Json 表属性 数据存储及格式化---------*/
	...
	function formateJson_query(){
		...
		for(var i=0;i<selects.length;i++){
			...
			//添加组件ID
			field.add('id',selects[i]['a_data_name']);
...
function formateJson_query_attr(){
 ...
 /*-------------3.0 公共属性-----------------------------*/
	...
	//按钮绑定交易的交易列表
	function formatterQueryButtonTran(value, row, index) {
		console.info('index='+index);
		 var _id = "interface-query"+row.btnId+"_"+index;
		 var tranOptions = '';
		 for(var i=0;i<$query_data.trans.table.interface.length;i++){
			 tranOptions += '<option value="'+$query_data.trans.table.interface[i].id+'">'+$query_data.trans.table.interface[i].name+'</option>';
		 }
		 return [
			 '<select id="'+_id+'" auto-interface-query="true" data-role="multiselect" >',
			 tranOptions,				
			'</select>'
	   ].join('');
	}
	 ...
	 function def_pub_attr_query(){
	 	 ...
	 	 $('select[auto-c-type-query]').multiselect({
	 		 onChange: function(option,checked,select) {
	 			 ...
	 			 //TODO 切换展开内容
	 			 $table_query.bootstrapTable('collapseRow', tr.data('index'));
	 			 $table_query.bootstrapTable('expandRow', tr.data('index'));
	 		 }
	 	 });
	 	 //2.0 只读
	 ...
	 function def_button_attr_query(){
	 	 //TODO 切换组件类型的时候需要个性化属性面板 
	 	
	 	 //交易绑定
	 	 $('select[auto-interface-query]').multiselect({
	 		 onChange: function(option,checked,select) {
	 			$('#table_query_buttons').bootstrapTable('resetView');
	 			var $rowId = this.$select.parent().parent().parent().data('index');
	 			console.info('tr.index='+$rowId);
	 			var tranId = $(option).val();
	 			var tr = getRowByIndex_queryButton($rowId);
	 			var ID = tr.btnId;
	 			var attribute = undefined;
	 			 if(queryButtonAttrMap.containsKey(ID)){
	 				 attribute = queryButtonAttrMap.get(ID);
	 			 }else{
	 				attribute = new Hashtable();
	 				queryButtonAttrMap.add(ID,attribute);
	 			 }
	 			 attribute.add('btnId',ID);
	 			 attribute.add('btnName',tr.btnName);
	 			 attribute.add('tranId',tranId);
	 			 if($query_data.trans.table.interface!=undefined){
	 				 for(var i=0;i<$query_data.trans.table.interface.length;i++){
	 					 if(tranId==$query_data.trans.table.interface[i].id){
	 						attribute.add('interface', $query_data.trans.table.interface[i]);
	 						break;
	 					 }
	 				 }
	 			 }
...
function getModule_01_ButtonLabel(val){
	if(val=='query'){
		return '查询按钮';
	}else if(val=='delete'){
		return '删除按钮';
...
function getRowByIndex_queryButton(index){
	 var ID = $('#table_query_buttons').find('tbody').children('tr').eq(index).children('td').eq(1).text();
	 return $('#table_query_buttons').bootstrapTable('getRowByUniqueId',ID);
}
