/* ========================================================================
 * Rainbow: module_01_form.js v1.0.0 
 * Author: chenyl
 * Time: 20170517
...
var formButtonAttrMap = new Hashtable();
...
var PrivateAttrFormMap = new Hashtable();
 ...
 $(function () {
	 console.info('form isInitModule='+isInitModule);
	 if(isInitModule){
		 init_form();
 ...
 function init_form(){
	 ...
	 $.ajax({
		   ...
		   success: function(data){
			   ...
			   $form_data = data;
			   $('#table_form').bootstrapTable('load',data.trans.table.field);
		   }
	});
	...
	$('#selectId_906550').multiselect({
		 onChange: function(option,checked,select) {
			 ...
			 if(formButtonArrayList.size()>0){
				 var data = new Array();	
				 formButtonAttrMap.clear();	//清理按钮与交易绑定映射
				 for(var i=0;i<formButtonArrayList.size();i++){
					 //按钮关联接口列表显示
					 var r = {
						"btnId":formButtonArrayList.get(i),
						"btnName":getModule_01_ButtonLabel(formButtonArrayList.get(i)),
						"interface":$form_data.trans.table.interface[0].id
					 };
					 data.push(r);
					 
					//重新初始化页面按钮的交易绑定
					 var attribute = new Hashtable();
		 			 attribute.add('btnId',formButtonArrayList.get(i));
		 			 attribute.add('btnName',getModule_01_ButtonLabel(formButtonArrayList.get(i)));
		 			 attribute.add('tranId',$form_data.trans.table.interface[0].id);
		 			 attribute.add('interface', $form_data.trans.table.interface[0]);
		 			 formButtonAttrMap.add(formButtonArrayList.get(i),attribute);
				 }
				 console.info('data='+data);
				 $('#table_form_buttons').bootstrapTable('load',data);
			 }
		 }
	 });
	
	//初始化按钮绑定交易table
	 $('#table_form_buttons').bootstrapTable({
		 onPostBody:function(){
			 def_button_attr_form();
		 },
		 onEditableSave:function(field, row, oldValue, $el){
			 $('#table_form_buttons').bootstrapTable('resetView', {
	         });
		 },
		 onExpandRow:function(index, row, $detail){

			 var attr = formButtonAttrMap.get(row.btnId);
			 
			 var ifcId = attr.interface;

			 if(ifcId==='save'){/*保存按钮请求与响应报文映射配置*/
				 onExpand_interface(index, row, $detail,'saveBtn');
			 }
		 }
	 });
 ...
 /*--------------2.0 formate datagrid attr Json 表属性 数据存储及格式化---------*/
	...
	function formateJson_fome(){
		...
		for(var i=0;i<selects.length;i++){
			...
			//添加组件ID
			field.add('id','EDIT_'+selects[i]['a_data_name']);
...
function formateJson_form_attr(){
 ...
 /*-------------3.0 公共属性-----------------------------*/
	...
	//按钮绑定交易的交易列表
	function formatterFormButtonTran(value, row, index) {
		console.info('index='+index);
		 var _id = "interface-form"+row.btnId+"_"+index;
		 var tranOptions = '';
		 for(var i=0;i<$form_data.trans.table.interface.length;i++){
			 tranOptions += '<option value="'+$form_data.trans.table.interface[i].id+'">'+$form_data.trans.table.interface[i].name+'</option>';
		 }
		 return [
			 '<select id="'+_id+'" auto-interface-form="true" data-role="multiselect" >',
			 tranOptions,				
			'</select>'
	   ].join('');
	}
	 ...
	 function def_button_attr_form(){
	 	 //TODO 切换组件类型的时候需要个性化属性面板 
	 	
	 	 //交易绑定
	 	 $('select[auto-interface-form]').multiselect({
	 		 onChange: function(option,checked,select) {
	 			$('#table_form_buttons').bootstrapTable('resetView');
	 			var $rowId = this.$select.parent().parent().parent().data('index');
	 			console.info('tr.index='+$rowId);
	 			var tranId = $(option).val();
	 			var tr = getRowByIndex_formButton($rowId);
	 			var ID = tr.btnId;
	 			var attribute = undefined;
	 			 if(formButtonAttrMap.containsKey(ID)){
	 				 attribute = formButtonAttrMap.get(ID);
	 			 }else{
	 				attribute = new Hashtable();
	 				formButtonAttrMap.add(ID,attribute);
	 			 }
	 			 attribute.add('btnId',ID);
	 			 attribute.add('btnName',tr.btnName);
	 			 attribute.add('tranId',tranId);
	 			 if($form_data.trans.table.interface!=undefined){
	 				 for(var i=0;i<$form_data.trans.table.interface.length;i++){
	 					 if(tranId==$form_data.trans.table.interface[i].id){
	 						attribute.add('interface', $form_data.trans.table.interface[i]);
	 						break;
	 					 }
	 				 }
	 			 }
 ...
 function onExpand_interface(index, row, $detail,sign){
	 var tabInterfaceId = '#table_'+sign;
...
function getRowByIndex_formButton(index){
	 var ID = $('#table_form_buttons').find('tbody').children('tr').eq(index).children('td').eq(1).text();
	 return $('#table_form_buttons').bootstrapTable('getRowByUniqueId',ID);
}
/* function getHeight() {
