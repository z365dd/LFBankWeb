...
- (enh #978, #974): Implement exif restoration for resized images via [`piexif` plugin](/hMatoba/piexifjs).
...
- (enh #643):Implement rearranging / sorting functionality for initial preview. 
    - Add ability to rearrange and sort thumbnails by drag & drop. This feature will use the [Sortable plugin](/RubaXa/Sortable) which will be included in the `js/plugins` folder.
...
- Relocate sample files from examples directory to [bootstrap-fileinput-samples](/kartik-v/bootstrap-fileinput-samples) repo.
