    ...
    <body>
        ...
        <script type="text/javascript">
            ...
            $(function(){
                ...
                messenger = new Messenger('CSTEditerWindow', 'faceui');
                messenger.listen(function(msg){
                    if (typeof msg === "string") {
                        ...
                        if (msg.message === "CSTURL") {
                            ...
                            console.info(path);
                            setJspUrl(path);
                            getCST(path, msg.numark, msg.quesbtn, msg.idname);
                ...
                function save() {
                	if (cstUrl.endsWith(".jsp") && isChange) {
                        var value = editor.getValue();
                        setCST(value, cstUrl);
                   	 	isChange = false;
                    }
            ...
            function setCST(context, path){
                ...
                $.ajax({
                    ...
                    url: ctxIde+'/ide/setCst',
                    dataType: "json",
                    success: function(data){
                        if (data.msg === "success") {
                            sendMessage('Parent', 'savescs_cst');
                        }
                        else {
                            sendMessage('Parent', 'saverr_cst');
                        }
                    }
                });
            ...
            function getCST(path, numark, btn, idname){
                $.ajax({
                    type: "POST",
                    data: {
                        path: path
                    },
                    url: ctxIde+'/ide/getCst',
                    dataType: "json",
                    success: function(data){
                        if (data.msg === "SCUESS") {
                            editor.setValue(data.cstContent);
                            var length = editor.session.getLength();
                            if (numark != undefined) {
            ...
            var JqueryMethod = {
                ...
                onDblClickRow:'\t//双击事件\n\t$("#table").on(\'dbl-click-row.bs.table\', function (row, $element) {\n\n\t});\n',
               	...
               	onCheckSome:'\t//部分行选择事件\n\t$("#table").on(\'check-some.bs.table\', function (rows) {\n\n\t});\n',
               	onUncheckSome:'\t//对部分行取消选择事件\n\t$("#table").on(\'uncheck-some.bs.table\', function (rows) {\n\n\t});\n',
