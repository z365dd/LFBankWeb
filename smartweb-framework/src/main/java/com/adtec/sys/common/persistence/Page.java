...
public class Page<T> {
	...
	private int pageSize = Integer.valueOf(ParamUtil.getPageSize()); // 页面大小，设置为“-1”表示不进行分页（分页无效）
	...
	private boolean isBooststrap = false;  //是否属于booststrap-table请求
	
	private String pgSide = "0";	//booststrap-table请求时,分页方向0-客户端，1-服务器
	...
	public Page(HttpServletRequest request, HttpServletResponse response, int defaultPageSize){
		...
		String pgside = request.getParameter("pgside");
		if(StringUtils.isNotBlank(pgside)){
			isBooststrap = true;
			if("server".equalsIgnoreCase(pgside)){
				pgSide = "1";
			}else{
				pgSide = "0";
				defaultPageSize = -1;
...
//		}
		...
		if(isBooststrap){
			orderBy = request.getParameter("sort")+" "+request.getParameter("order");
		}
...
//		sb.append(funcName+"("+pageNo+",this.value,'"+funcParam+"');\" onclick=\"this.select();\"/> 条，");
		...
		sb.append(fromIndex + " / ");
		sb.append(toIndex > count ? count : toIndex + " 条，");
...
//		sb.insert(0,"<div class=\"page\">\n").append("</div>\n");
	...
	public boolean isBooststrap() {
		return isBooststrap;
	}

	public void setBooststrap(boolean isBooststrap) {
		this.isBooststrap = isBooststrap;
	}

	public String getPgSide() {
		return pgSide;
	}

	public void setPgSide(String pgSide) {
		this.pgSide = pgSide;
	}
