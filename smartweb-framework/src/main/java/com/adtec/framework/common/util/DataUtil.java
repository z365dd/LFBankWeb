...
public class DataUtil {
	...
	public static String normalToFinal(double data) {
		try {
			// 该方法为重载函数：格式转换统一调用normalToFinal(String data)
	...
	public static String setCharAt(String str, int index, char ch) {
		if (str == null) {
			throw new BaseException(SysErr.E_MESSAGE,"字符串为null");
		}
		String newStr = str;
		if (index >= str.length()) {
	...
	public static String getCharAt(String str, int index) {
 	...
 	/**
 	 * 判断字符串，字符串为空则返回空字符串，字符串不为空则返回原字符串
 	 * 
 	 * @param str 
 	 * 			-需要判断的字符串
 	 * @return
 	 * 			- 字符串为空，则返回"", 否则则返回原字符串
 	 */
 	public static String chgNullToEmpty(String str) {
