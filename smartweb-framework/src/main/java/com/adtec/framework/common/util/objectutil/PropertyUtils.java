...
//		}
	...
	/**
	 * An improved version of {@link #write(Object, String, Object)} where the
	 * value starts as a string and is converted to the correct property type
	 * before being assigned.
	 * 
	 * 
	 */
	public static void smartWrite(Object target, String propertyName,
			String value) {
		ClassAdaptor a = getAdaptor(target);
	...
	/**
	 * Initializes the properties of an object from a string. The string is a
	 * comma-seperated sequence of property names and values. Property names are
	 * seperated from values be an equals sign. Spaces before and after the
	 * property names are trimmed. For boolean properties, the equals sign and
	 * value may be omitted (a value of true is assumed), or the property name
	 * may be prefixed with an exclamation point to indicated false value.
	 * Example: <code>validate,maxLength=10,displayName=User Id</code>.
	 * 
	 * @param target
	 *            the object to be configured
	 * @param initializer
	 *            the string encoding the properties and values to be configured
	...
	/**
	 * Returns true of the instance contains a writable property of the given
	 * type.
	 * 
	 * @param target
...
//			try {
//				return BeanUtils.getProperty(target, propertyName);
//			} catch (IllegalAccessException e) {
//				// TODO Auto-generated catch block
//				e.printStackTrace();
//			} catch (InvocationTargetException e) {
...
//			return null;
	...
	/**
	 * Returns the {@link PropertyAdaptor} for the given target object and
	 * property name.
	 * 
	 * @throws ApplicationRuntimeException
	 *             if the property does not exist.
	 */
	public static PropertyAdaptor getPropertyAdaptor(Object target,
			String propertyName) {
		ClassAdaptor a = getAdaptor(target);
	...
	/**
	 * Returns an unordered List of the names of all readable properties of the
	 * target.
	 */
	 ...
	 * target.
	 */
	public static List getReadableProperties(Class<?> target) {
		return getAdaptor(target).getReadableProperties();
	}

	/**
	 * Returns an unordered List of the names of all writable properties of the
	 * target.
	 */
	...
	private static ClassAdaptor getAdaptor(Object target) {
		...
		synchronized (INTROSPECTOR_MUTEX) {
			ClassAdaptor result = (ClassAdaptor) _classAdaptors
					.get(targetClass);
	...
	private static ClassAdaptor buildClassAdaptor(Object target,
			Class targetClass) {
		try {
			...
			throw new RuntimeException("Unable to Introspect " + targetClass,
					ex);
		}
	}
