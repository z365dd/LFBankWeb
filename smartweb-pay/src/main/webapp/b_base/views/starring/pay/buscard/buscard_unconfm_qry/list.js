...
const btnInfos = [{text: '确认成功', act: 'confmSucc'},{text: '确认失败', act: 'confmFail'}];

function action(value, row, index) {
    let btnhtml = '';
    btnInfos.forEach(btnInfo => {
        btnhtml += `<button class='unformatter lb1' style="width: 35%" onclick="${btnInfo.act}('${Base64.encode(JSON.stringify(row))}')">${btnInfo.text}</button>`
    });
    return btnhtml;
}
//确认成功
function confmSucc(row) {
    let decode = JSON.parse(Base64.decode(row));
    const data = {
        busiNo:getS('busiNo'),
        platDate:decode.plat_DATE,
        platSeq:decode.plat_SEQ,
        //10-确认成功
        confmStat:'10'
    }
    sendPostOfAjax(ctx + '/bus/data/confmDeal', data, false, true, () => {
        showContent('确认状态已修改!','success');
        refreshTable();
    })

}
//确认失败
function confmFail(row) {
    let decode = JSON.parse(Base64.decode(row));
    const data = {
        busiNo:getS('busiNo'),
        platDate:decode.plat_DATE,
