...
$(function () {
    ...
    const BUSI_NO = url.searchParams.get('busi_no');
    const RULE_NO = url.searchParams.get('rule_no');
    const CLR_TP = url.searchParams.get('clr_tp');
    const CHK_TP = url.searchParams.get('chk_tp');
    const RFND_TP = url.searchParams.get('rfnd_tp');
    const NOTE_TP = url.searchParams.get('note_tp');
    const DAY_NOTE_TP = url.searchParams.get('day_note_tp');
    const TEMP_ACCT = url.searchParams.get('temp_acct');
    const TEMP_ACCT_NAME = url.searchParams.get('temp_acct_name');
    ...
    //日切通知类型
    dayNoteTpMap = dayNoteTpMapSelect($("select[name='DAY_NOTE_TP']"))
    setI("TEMP_ACCT", TEMP_ACCT);
    setI("TEMP_ACCT_NAME", TEMP_ACCT_NAME);
    setI("RULE_NO", RULE_NO);
    setI("BUSI_NO", BUSI_NO);
    setS("CLR_TP", CLR_TP);
    setS("CHK_TP", CHK_TP);
    setS("RFND_TP", RFND_TP);
    setS("NOTE_TP", NOTE_TP);
    setS("DAY_NOTE_TP", DAY_NOTE_TP);
    busiNo();
...
function busiNo() {
    ...
    setI("busiName", busiM.get(getI("BUSI_NO")));
}
...
function save() {
    confirmx('是否保存', function () {
        ...
        sendPostOfAjax(ctx + '/clearRule/data/update', data, false, true, () => {
