...
let busiNoFlg = false;
let busiNameFlg = false;
let acctFlg = false;
let acctNameFlg = false;
let phoneNoFlg = false;
let nameFlg = false;
...
$(function () {
    ...
    $("#busiName").on("blur", function (event) {
        ...
        if (busiName.length > 120) {
                busiNameFlg = false;
                $("#busiNameErrTip").text("业务名称长度不能超过120位");
                $("#busiNameErrTip").show();
                return;
            }
        busiNameFlg = true;
    ...
    $("#payAcct").on("blur", function (event) {
        ...
        if (payAcct.length > 64) {
            if (validate.test(payAcct)) {
                acctFlg = false;
                $("#acctErrTip").text("清算账户长度不能超过64位");
                $("#acctErrTip").show();
                return;
            }
        }
    ...
    $("#payAcctName").on("blur", function (event) {
        ...
        if (payAcctName.length > 300) {
            if (validate.test(payAcctName)) {
                acctNameFlg = false;
                $("#acctNameErrTip").text("清算账户名称长度不能超过300位");
                $("#acctNameErrTip").show();
                return;
            }
        }
    ...
    $("#phoneNo").on("blur", function (event) {
        ...
        if (phoneNo.length > 0) {
            if (!mobileReg.test(phoneNo) && !telReg.test(phoneNo)) {
                phoneNoFlg = false;
        ...
        if (phoneNo.length > 14) {
                phoneNoFlg = false;
                $("#phoneNoErrTip").text("咨询电话长度不能超过14位");
                $("#phoneNoErrTip").show();
                return;
            }
        phoneNoFlg = true;
...
function save() {
    confirmx('是否保存', function () {
        ...
        //保存
        data.operStat = '1';
        sendPostOfAjax(ctx + '/ykt/data/merModify', data, false, true, () => {
            //添加成功
...
//     const info = sendGetOfAjax(ctx + "/prod/oper/entrDemo/area?parentId=1", false);
//     console.info(info);
//     info.forEach(item => {
//         areas.set(item.id, item.name);
//     });
