...
const initInstTable = () => {
    const url = ctx + '/offline/data/merQry';
...
function update(row) {
    ...
    parent.window.$('a[href^=\'#tab_update\']').attr('url', ctx + `/offline_mer_qry//page/update`);
...
function expExcel() {
    ...
    top.$.jBox.confirm("是否要导出商户信息?", "系统提示", function (v, h, f) {
        if (v == "ok") {
            showLoading();
            $("#from").attr("action", ctx + "/offline/data/exportMerData");
            $("#from").submit();
            $.ajax({
                url: ctx + '/offline/data/exportMerData',
                type: 'POST',
